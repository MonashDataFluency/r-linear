...
# include an intercept, using a formula like ~ 0 + group. This makes it
# easier to write out contrasts, at least for simple experimental
# designs.
#
# As we discussed earlier, this doesn't make a difference to the
# results. Well, it shouldn't make a difference to the results. Because
# of a calculation shortcut the limma-voom method takes when using
# weights, there can be very slight differences if using this method,
# with perfectly accurate results if ~ 0 + group is used and very
# slightly inaccurate results if ~ group is used. For more complex
# designs, such as designs that account for a batch effect,
# contrastAsCoef() could potentially be used to get perfectly accurate
# results with the limma-voom method. See the note in the documentation
# for contrasts.fit. I have never seen anyone bother to do this in
# practice though!
#
# For example our PVC dataset earlier would traditionally be set up like
# this.
...
# It is a very common pattern in RNA-Seq experiments to have a set of
# treatment groups and a set of batches, with all of the treatments
# performed once in each batch. Including the batch in the model then
# often increases the statistical power to detect differences between
# groups.
...
#
# 2. Differ in expression at time 0.5 between the lower and upper
