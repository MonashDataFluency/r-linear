...
* Excessive step-wise comparison of models like in this challenge raises problems similar to multiple testing, and can be criticized for not properly controlling statistical significance. If you have nuisance variables that you are not sure if you need to adjust for, it is ok to leave them in a model whether or not they appear significant.

* An altenative approach is to rank models by the Akaike Information Criterion (AIC) or similar. AIC lets you compare models even if they are not nested (but you still can't compare different transformations of the response variable).
