...
## Single factor predictor, more levels
...
`summary` also outputs p-values. Too many p-values. It doesn't respect the hierarchy of terms in the model. The p-value for dropping the intercept is nonsense with the age term still present:
...
A difference between `glht` and limma's `contrasts.fit` is that limma uses columns as contrasts, rather than rows.
