...
## A different way to test hypotheses: contrasts

A **contrast** is a linear combination of coefficients.

Constraining a contrast to be zero is another way to specify a null hypothesis.

Contrasts offer great flexibility, for example:

>* difference of response between any two levels in a factor
>* difference of response between two imagined individuals

From a contrast we obtain:

>* estimated value (calculate with dot product)
...
A p-value is a uniformly random number between 0 and 1, if the null hypothesis is true.
...
However suppose we perform 20 such tests. There is a high chance we will falsely reject at least one of them.
...
>* test a hypothesis and calculate a CI using a contrast
...
>* Then realize data is always over-dispersed,<br>switch to safer Quasi-Likelihood F-test
