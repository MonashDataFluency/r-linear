...
Ok, we can reject a simple model using a more complex model, 
<br>but maybe we may simply have enough data to detect subtle and unimportant differences.

How can we quantify how good a model is?
...
With a single predictor, $R^2$ is exactly the Pearson correlation $r^2$.
...
> * To account for this use *adjusted* $R^2$, also reported by `summary( )`.
...
> * Partial and part $R^2$.

Mental exercise: reconcile this variance pool view with our geometric view of model fitting.
...
**What if the hypotheses are mutually exclusive?**

Then only one can be true, so no problem!
