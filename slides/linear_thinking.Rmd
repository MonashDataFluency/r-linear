...
The noise is assumed to be normally distributed with standard deviation $\sigma$ (variance $\sigma^2$):
...
Our starting point for modelling is always a data frame, but the process of getting $y$ and $X$ from the data frame has some complications, and a special "formula" syntax.

```
        data frame

            ||
            || Magic involving "~" syntax happens
            \/
...
A linear combination of coefficients, for example:

>* difference of response between two levels in a factor
>* predicted response for a certain individual
>* difference in predicted response between two individuals

Constraining a contrast to be zero is another way to specify H0.

>* especially useful to say "there is no difference between these two levels"

Can obtain:

>* estimated value with dot product
...
$n_\text{levels}-1$ columns, "one-hot" encoding. <span class="tip">*<span class="tiptext">The encoding can be adjusted using the `C( )` function within a formula. The default differs between S and R languages!</span></span>
...
>* Partial Least Squares (PLS) <br>(naive best directions first)
>* Principal Component Regression (PCR)<br>(largest variation in X first)

### Noise not normal (eg binary or count data)

>* First use Generalized Linear Models: `glm( )`<br>(logistic regression, binomial, Poisson, negative binomial, gamma, etc)
>* Realize data is over-dispersed and switch to Quasi-Likelihood

### Modelling populations, repeated measurements on individuals
...
### Survival time data, individuals drop out of study or haven't died yet

>* Cox survival model: `survival::coxph( )`

### Prior knowledge is important, or need a flexible hierarchical model
...
>* Neural network<br>(plug together many logistic regression models)
>* Tree methods, such as gradient boosted trees
