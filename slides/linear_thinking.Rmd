...
A **linear hypothesis** test tests an H0 in which some linear combination of coefficients in H1 is constrained to be zero.

Specified differently but equivalent to an `anova( )` test.

From a linear hypothesis we obtain:
...
>* a **contrast** compares specific levels of an experimental factor<span class="tip">*<span class="tiptext">Terminology is a little tricky here. The limma package we are using later refers to linear hypotheses in general as contrasts.</span></span>
